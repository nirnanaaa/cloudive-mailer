...
// PrintConfigCommand represents the command executed by "cloudive-mailer config".
...
func (cmd *PrintConfigCommand) Run(args ...string) error {
	...
	if err := config.Validate(); err != nil {
		return fmt.Errorf("%s. To generate a valid configuration file run `cloudive-mailer config > cloudive.generated.conf`", err)
	}
...
Usage: cloudive-mailer<prg> config [flags]
    -config <path>
