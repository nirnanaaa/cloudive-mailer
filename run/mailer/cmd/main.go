...
func (cmd *Command) Run(args ...string) error {
	...
	if err := config.Validate(); err != nil {
		return fmt.Errorf("%s. To generate a valid configuration file run `roove-thumber config > roove-thumber.generated.conf`", err)
	}
...
var usage = `Runs the roove-thumber Listener server.
...
//   2. The environment variable ROOVE_CONFIG_PATH
...
func (opt *Options) GetConfigPath() string {
	...
	} else if envVar := os.Getenv("ROOVE_CONFIG_PATH"); envVar != "" {
		return envVar
	}

	for _, path := range []string{
		os.ExpandEnv("${HOME}/.roove/thumber.conf"),
		"/etc/roove/thumber.conf",
	} {
